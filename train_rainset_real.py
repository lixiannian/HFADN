...
from network.HFADN import HFADN
...
class Session:
    def __init__(self):
        ...
        if len(settings.device_id) > 1:
            self.net = nn.DataParallel(HFADN()).cuda()
        else:
        ...
        self.per_loss = PerLoss(vgg_model).cuda()
        self.step = 1
        ...
        self.sche_net = torch.optim.lr_scheduler.CosineAnnealingLR(self.opt_net, T_max =settings.epoch, eta_min=1e-5)
    ...
    def load_checkpoints_net(self, name):
        ...
        try:
            ...
            obj = torch.load(ckp_path, weights_only=True)
    ...
    def inf_batch(self, name, batch):
        ...
        loss_per = self.per_loss(syn_derain, B)
        ...
        if self.ssim_loss == True:
            ...
            kl_streak_loss = 0.0
            for i in range(len(syn_code)):
                logp_syn_code = F.log_softmax(syn_code[i])
                p_real_code = F.softmax(real_code[i])
                kl_streak_loss += F.kl_div(logp_syn_code, p_real_code, reduction='batchmean')
            kl_streak_loss /= len(syn_code)
            loss = recon_loss + 1e-3 * kl_streak_loss + 1e-2 * loss_per
        else:
            loss = l1_loss
        ...
        print(f'\rtrain---step: {self.step} loss : {loss.item():.10f} '
              ...
              f'per_loss : {loss_per.item():.10f} | ssim: {ssim.item():.5f}', end='', flush=True)
...
def run_train_val(ckp_name_net='net_latest'):
    ...
    while sess.step < settings.total_step + 1:
        ...
        if sess.step % (settings.one_epoch) == 0:
            ...
            logfile = open('./log_test/' + 'HFADN-real' + '.txt', 'a+')
