...
class Session:
    ...
    def inf_batch(self, name, batch):
        ...
        ssim_loss = 1-self.ssim(derain, B)
        per_loss = self.per_loss(derain, B)
        
        loss = l1_loss + 0.5 * ssim_loss + 0.5 * per_loss
        ...
        if name == 'train':
            ...
            print(f'\rtrain---step: {self.step} loss : {loss.item():.5f} | ssim: {ssim_loss.item():.5f} '
                  f' per: {per_loss.item():.5f} | lr: {lr:.8f}', end='', flush=True)
...
def run_train_val(ckp_name_net='net_520poch'): # 'net_latest'
    ...
    while sess.step < settings.total_step + 1:
        ...
        if sess.step % (settings.one_epoch*10) == 0:
            ...
            logfile = open('./log_test/' + 'HFADN' + '.txt', 'a+')
